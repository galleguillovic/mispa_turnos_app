...
class VistaTurnos(tk.Frame):
    ...
    def _obtener_servicios(self):
        ...
        try:
            cursor = conexion.cursor(dictionary=True)
            cursor.execute("""
                SELECT id_servicio, nombre, precio, duracion, id_especialidad
    ...
    def _obtener_empleados_por_especialidades(self, ids_especialidades):
        ...
        try:
            cursor = conexion.cursor(dictionary=True)
            placeholders = ",".join(["%s"] * len(ids_especialidades))
            cursor.execute(f"""
                SELECT e.id_empleado, p.nombre, p.apellido
                FROM empleados e
                JOIN personas p ON e.id_persona = p.id_persona
                ...
                WHERE e.activo = 1
                  AND ee.id_especialidad IN ({placeholders})
                GROUP BY e.id_empleado
                HAVING COUNT(DISTINCT ee.id_especialidad) = %s
                ORDER BY p.apellido
            """, ids_especialidades + [len(ids_especialidades)])
            return cursor.fetchall()
        except Exception as e:
    ...
    def _mostrar_listado(self):
        ...
        anchos = {
            "dia": 120, "hora": 70, "servicio": 180,
            "cliente": 140, "empleado": 140,
    ...
    def _mostrar_detalles(self, turno):
        ...
        tk.Button(frame_btns, text="Cancelar turno",
    ...
    def _confirmar_cancelar(self, turno):
        ...
        tk.Label(popup,
                 ...
                 bg="#EEEEEE", fg="#333333",
                 font=("Poppins", 11), justify="center").pack(pady=(25, 15))

        frame_btns = tk.Frame(popup, bg="#EEEEEE")
        frame_btns.pack()

        tk.Button(frame_btns, text="Eliminar", bg="white", fg="black",
    ...
    def _construir_formulario(self, modo, turno=None):
        ...
        tk.Label(frame_f1, text="Fecha:*", bg="white", fg="#333333",
                 font=("Poppins", 11)).grid(row=0, column=0, sticky="w")
        tk.Label(frame_f1, text="Hora:*", bg="white", fg="#333333",
                 font=("Poppins", 11)).grid(row=0, column=1, sticky="w", padx=(20, 0))

        self.date_entry = DateEntry(frame_f1, font=("Poppins", 11),
                                    date_pattern="dd/mm/yyyy",
                                    background="#D68092", foreground="white",
                                    borderwidth=0, locale="es_AR")
        self.date_entry.grid(row=1, column=0, sticky="ew", ipady=6, pady=(4, 0))

        frame_hora = tk.Frame(frame_f1, bg="white")
        frame_hora.grid(row=1, column=1, sticky="ew", pady=(4, 0), padx=(20, 0))

        self.spin_hora = ttk.Spinbox(frame_hora, from_=0, to=23, width=3,
        ...
        tk.Label(frame_f2, text="Servicio/s:*", bg="white", fg="#333333",
                 font=("Poppins", 11)).grid(row=0, column=0, sticky="w")
        tk.Label(frame_f2, text="Empleado:*", bg="white", fg="#333333",
                 font=("Poppins", 11)).grid(row=0, column=1, sticky="w", padx=(20, 0))
        ...
        self.listbox_servicios = tk.Listbox(
            ...
            selectbackground="#D68092", selectforeground="white")
        ...
        self.empleados_data = {
            ...
            for e in self._todos_empleados
        }
        ...
        tk.Entry(frame_cli, textvariable=self.var_cliente_buscar,
                 font=("Poppins", 11), bg="#EEEEEE", bd=0, relief="flat"
                 ).pack(fill="x", ipady=8)

        self.listbox_clientes = tk.Listbox(
            frame_cli, font=("Poppins", 10),
            bg="#EEEEEE", bd=0, height=4, exportselection=False,
            selectbackground="#D68092", selectforeground="white")
        ...
        tk.Label(frame_f3, text="Duración del turno:*", bg="white",
                 fg="#333333", font=("Poppins", 11)).grid(row=0, column=0, sticky="w")
        tk.Label(frame_f3, text="Seña pagada:", bg="white",
                 fg="#333333", font=("Poppins", 11)).grid(row=0, column=1, sticky="w", padx=(20, 0))
        ...
        tk.Button(frame_btns, text="Cancelar", bg="#D68092", fg="white",
                  font=("Poppins", 11), bd=0, relief="flat", cursor="hand2",
                  command=self._mostrar_listado
                  ).pack(side="left", ipadx=15, ipady=8, padx=(0, 10))

        texto_btn = "Aceptar" if modo == "agregar" else "Guardar cambios"
        tk.Button(frame_btns, text=texto_btn, bg="#D68092", fg="white",
                  font=("Poppins", 11), bd=0, relief="flat", cursor="hand2",
    ...
    def _actualizar_combo_empleados(self):
        ...
        ids_especialidades = list({
            ...
            if self.servicios_data[nombres_lista[i]].get("id_especialidad")
        })
        ...
        nuevos_valores = list(self.empleados_data.keys())
        self.combo_emp.configure(values=nuevos_valores)

        if self.var_empleado.get() not in self.empleados_data:
    ...
    def _guardar(self, modo, turno=None):
        ...
        if not self._verificar_disponibilidad(
                fecha_hora, duracion_min, id_empleado, ids_servicios, excluir):
        ...
        try:
            ...
            if modo == "agregar":
                cursor.execute("""
                    INSERT INTO turnos
                    (id_cliente, id_empleado, fecha_hora, estado,
                     precio_total, sena_pagada, duracion)
                    VALUES (%s, %s, %s, 'programado', %s, %s, %s)
                """, (id_cliente, id_empleado, fecha_hora,
                      precio_total, sena_pagada, duracion_total))
                id_turno = cursor.lastrowid
            else:
                cursor.execute("""
                    UPDATE turnos SET
                        id_cliente=%s, id_empleado=%s, fecha_hora=%s,
                        precio_total=%s, sena_pagada=%s, duracion=%s
                    WHERE id_turno=%s
                """, (id_cliente, id_empleado, fecha_hora,
                      precio_total, sena_pagada, duracion_total,
                      turno["id_turno"]))
                ...
                cursor.execute(
                    "DELETE FROM turno_servicio WHERE id_turno = %s",
                    (id_turno,))

...
class PanelUsuario(tk.Toplevel):
    ...
    def _construir_sidebar(self):
        ...
        tk.Label(self.sidebar, text=self.usuario["nombre"],
                 bg="#D68092", fg="white",
                 font=("Poppins ExtraBold", 13)).pack()

        # Configurar perfil
        frame_config = tk.Frame(self.sidebar, bg="#D68092", cursor="hand2")
        frame_config.pack(pady=(4, 20))
        frame_config.bind("<Button-1>",
                          lambda e: self._abrir_configurar_perfil())

        if self.iconos_blancos.get("lapiz"):
            lbl_lap = tk.Label(frame_config,
                               image=self.iconos_blancos["lapiz"],
                               bg="#D68092", cursor="hand2")
            lbl_lap.pack(side="left", padx=(0, 4))
            lbl_lap.bind("<Button-1>",
                         lambda e: self._abrir_configurar_perfil())

        lbl_cfg = tk.Label(frame_config, text="Configurar perfil",
                           bg="#D68092", fg="white",
                           font=("Poppins", 9), cursor="hand2", underline=0)
        ...
        if self.iconos_blancos.get("cerrar"):
            lbl_ico = tk.Label(btn_cerrar,
                               image=self.iconos_blancos["cerrar"],
                               bg="#D68092", cursor="hand2")
            lbl_ico.pack(side="left", padx=(10, 8), pady=8)
            lbl_ico.bind("<Button-1>", lambda e: self._cerrar_sesion())

        lbl_cerrar = tk.Label(btn_cerrar, text="Cerrar sesión",
                              bg="#D68092", fg="white",
    ...
    # INICIO (Calendario + Turnos del empleado) 
    ...
    def _mostrar_inicio(self):
        ...
        tk.Label(frame_centro, text="Vista de Turnos",
                 bg="#F9F0F2", fg="#D68092",
                 font=("Poppins ExtraBold", 18)).pack(anchor="w")
        tk.Frame(frame_centro, bg="#D68092", height=2).pack(
            fill="x", pady=(4, 20))

        hoy = datetime.date.today()
        cal = Calendar(
            frame_centro, selectmode="day",
            year=hoy.year, month=hoy.month, day=hoy.day,
            background="#D68092", foreground="white",
            selectbackground="#A0526A",
            headersbackground="#D68092", headersforeground="white",
            weekendforeground="white", othermonthforeground="#cccccc",
            font=("Poppins", 9), showweeknumbers=False, locale="es_AR"
        )
        ...
        # Panel derecho: turnos programados del empleado logueado
        tk.Label(frame_derecha, text="Turnos Programados",
                 bg="white", fg="#D68092",
                 font=("Poppins ExtraBold", 14)).pack(
    ...
    def _cargar_turnos_proximos(self, frame_lista):
        ...
        try:
            ...
            """, (hoy, manana, self.usuario["id_usuario"]))
        ...
        for turno in turnos:
            self._crear_tarjeta_turno_inicio(frame_lista, turno)

    def _crear_tarjeta_turno_inicio(self, frame_padre, turno):
        ...
        self._foto_empleada_mini(frame_foto, turno.get("emp_foto"))
        ...
        tk.Label(frame_info, text=turno.get("servicios", "—"),
                 bg="white", fg="#666666",
                 font=("Poppins", 9), wraplength=150).pack(anchor="w")

        fecha = turno["fecha_hora"]
        fecha_str = fecha.strftime("%a. %d - %H:%Mhs") \
            if hasattr(fecha, "strftime") else str(fecha)
        tk.Label(frame_info, text=fecha_str, bg="white", fg="#666666",
        ...
        try:
            ...
            lbl_ojo.bind("<Button-1>",
                         lambda e, t=turno: self._ver_detalle_desde_inicio(t))
        except Exception:
            tk.Label(tarjeta, text="👁", bg="white",
                     font=("Poppins", 14), cursor="hand2").pack(
                side="right", padx=10)

    def _foto_empleada_mini(self, frame_padre, nombre_foto):
        size = 45
        try:
            if nombre_foto:
    ...
    def _ver_detalle_desde_inicio(self, turno):
        """Navega a Ver turnos y abre el detalle del turno seleccionado."""
        ...
        for widget in self.area_contenido.winfo_children():
            widget.destroy()
        self._mostrar_turnos(turno_detalle=turno)
    ...
    def _mostrar_turnos_dia(self, fecha, frame_padre):
        ...
        tk.Label(frame_padre,
                 text=f"Turnos del {fecha_str}:",
                 bg="#F9F0F2", fg="#D68092",
                 font=("Poppins ExtraBold", 11)).pack(anchor="w")

        if not turnos:
            tk.Label(frame_padre,
                     text="No tenés turnos ese día.",
                     bg="#F9F0F2", fg="#999999",
                     font=("Poppins", 10)).pack(anchor="w", pady=4)
            return

        for t in turnos:
            hora  = t["fecha_hora"].strftime("%H:%M")
            texto = f"• {hora}hs — {t.get('servicios', '—')}"
            tk.Label(frame_padre, text=texto,
                     bg="#F9F0F2", fg="#555555",
                     font=("Poppins", 10)).pack(anchor="w", pady=2)

    # VER TURNOS (lectura + finalizar/pagar) 

    def _mostrar_turnos(self, turno_detalle=None):
        from vistas.vista_turnos_usuario import VistaTurnosUsuario
        vista = VistaTurnosUsuario(self.area_contenido, self)
        if turno_detalle:
            vista._mostrar_detalles(turno_detalle)

    # VER CLIENTES (lectura)

    def _mostrar_clientes(self):
        from vistas.vista_clientes_usuario import VistaClientesUsuario
        VistaClientesUsuario(self.area_contenido, self)

    # VER EMPLEADOS (lectura) 

    def _mostrar_empleados(self):
        from vistas.vista_empleados_usuario import VistaEmpleadosUsuario
        VistaEmpleadosUsuario(self.area_contenido, self)

    # VER SERVICIOS (lectura)

    def _mostrar_servicios(self):
        from vistas.vista_servicios_usuario import VistaServiciosUsuario
        VistaServiciosUsuario(self.area_contenido, self)